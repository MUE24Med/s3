...
function handleBackNavigation(e) {
    ...
    // ✅ إذا لم تكن هناك حالة تنقل، اسمح بالخروج
    if (!currentState) {
        console.log('📱 لا توجد حالة - السماح بالخروج');
        return;
    }

    // ✅ منع السلوك الافتراضي فقط عند وجود حالة تنقل
    e.preventDefault();
    ...
    // ✅ معالجة حالة WOOD_VIEW - التحقق من وجود مجلدات
    if (currentState.state === NAV_STATE.WOOD_VIEW) {
        // ✅ إذا كنا داخل مجلد، ارجع للمجلد الأب
        ...
        // ✅ إذا كنا في الصفحة الرئيسية، ارجع لاختيار المجموعة
...
function setupBackButton() {
    ...
    window.addEventListener('popstate', (e) => {
        ...
        // ✅ إعادة إضافة state فقط إذا كان لدينا حالات تنقل
...
async function updateWoodInterface() {
    ...
    if (currentFolder === "") {
        ...
        // ✅ تحديث حالة التنقل للصفحة الرئيسية
        const currentState = getCurrentNavigationState();
        if (!currentState || currentState.state !== NAV_STATE.WOOD_VIEW) {
            navigationHistory = [];
