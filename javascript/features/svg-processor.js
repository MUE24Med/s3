...
function startHover() {
    ...
    const clipDefs = ensureDefs(mainSvg);
    ...
    if (bText) {
        ...
        let bbox;
        try { bbox = zText.getBBox(); } catch { bbox = { width: 0, height: 0 }; }
...
function wrapText(el, maxW) {
    ...
    words.forEach(word => {
        ...
        ts.textContent = test;
        let length = 0;
        try { length = ts.getComputedTextLength(); } catch { length = 0; }
...
function processRect(r) {
    ...
    if (name && name.trim() !== '') {
        ...
        let bbox;
        try { bbox = txt.getBBox(); } catch { bbox = { width: 0, height: 0 }; }
